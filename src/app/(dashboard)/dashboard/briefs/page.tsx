        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6">
            <div className={`flex-1 transition-all ${selectedBrief ? 'md:w-1/2 lg:w-1/3 block' : 'w-full block'}`}>
                ...
                {loading ? (
                    <div className="space-y-4">
                        {[...Array(4)].map((_, i) => (
                            <div key={i} className="h-24 rounded-2xl bg-gray-50 dark:bg-black border border-gray-100 dark:border-gray-800 animate-pulse" />
                        ))}
                    </div>
                ) : briefs.length > 0 ? (
                    <div className="space-y-4">
                        {briefs.map((brief) => (
                            <motion.div
                                ...
                                className={`p-6 rounded-2xl border transition-all cursor-pointer ${selectedBrief?.id === brief.id
                                        ...
                                        : "border-gray-200 dark:border-gray-800 bg-white dark:bg-black hover:border-orange/30"
                                    }`}
                            >
                                <div className="flex justify-between items-start mb-3">
                                    ...
                                    <span className="text-[10px] uppercase tracking-widest font-bold text-orange px-2 py-1 bg-orange/10 rounded-full">
                                        New
                                    </span>
                                </div>
                    ...
                    <div className="p-12 text-center rounded-3xl border border-dashed border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-black/50">
                        <div className="w-16 h-16 rounded-full bg-gray-100 dark:bg-lil-black text-[var(--text-muted)] flex items-center justify-center mx-auto mb-4">
                            <FiCheck size={24} />
                        </div>
            ...
            {selectedBrief && (
                <motion.div
                    ...
                    className="flex-[1.5] bg-white dark:bg-black border border-gray-200 dark:border-gray-800 rounded-3xl p-6 lg:p-8 sticky top-8 h-fit"
                >
                    <div className="flex items-center justify-between mb-8 pb-6 border-b border-gray-100 dark:border-gray-800">
                        <h2 className="text-2xl font-display text-[var(--text-primary)]">Brief Details</h2>
                        <button
                            onClick={() => setSelectedBrief(null)}
                            className="p-2 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors rounded-full hover:bg-gray-100 dark:hover:bg-lil-black"
                        >
                            <FiX size={20} />
                        </button>
                    </div>
                    ...
                    <div className="space-y-8 max-h-[60vh] overflow-y-auto pr-2 no-scrollbar">
                        {/* Client Info */}
                        <section>
                            <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4">Client Contact</h3>
                        ...
                        {/* Order Info */}
                        <section>
                            <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4">Order Summary</h3>
                            <div className="bg-gray-50 dark:bg-lil-black rounded-xl p-4 border border-gray-100 dark:border-gray-800">
                                ...
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${selectedBrief.retainer ? 'bg-green/10 text-green-500' : 'bg-gray-200 dark:bg-gray-800 text-[var(--text-muted)]'}`}>
                        ...
                        <section>
                            <h3 className="text-xs font-bold uppercase tracking-widest text-[var(--text-muted)] mb-4">Project Brief</h3>
                            <div className="space-y-4">
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100 dark:border-gray-800 flex gap-4">
                        <button
                            ...
                            className="flex-1 bg-orange text-white font-bold py-3 px-4 rounded-xl hover:bg-orange-light transition-all disabled:opacity-50 text-sm tracking-wide"
                        >
